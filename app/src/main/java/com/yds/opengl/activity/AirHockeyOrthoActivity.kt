 ...
 *
 */
class AirHockeyOrthoActivity : AppCompatActivity() {
    ...
    companion object{
        ...
        fun start(activity: AppCompatActivity){
            val intent = Intent()
            intent.setClass(activity, AirHockeyOrthoActivity::class.java)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (supportEs2){
            ...
            glSurfaceView?.setRenderer(AirHockeyOrthoRender())
            rendereSet = true
        }else {
...
class  AirHockeyOrthoRender : GLSurfaceView.Renderer{
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        uMarixLocation = GLES20.glGetUniformLocation(program, U_MATRIX)
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        //传递矩阵给做着色器
        uMarixLocation?.let {
            GLES20.glUniformMatrix4fv(it,1,false,projectMatrix,0)
        }
    }
